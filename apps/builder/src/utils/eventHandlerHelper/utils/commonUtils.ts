import { SectionViewShape } from "@flowagent-public/public-types"
...
export const setRouter = (params: { pagePath: string; viewPath?: string }) => {
  ...
  const isProductionMode = getIsFLOWAGENTProductMode(store.getState())
  ...
  if (index === -1) return
  const routerMatch = FLOWAGENTRoute.state.matches[0]
