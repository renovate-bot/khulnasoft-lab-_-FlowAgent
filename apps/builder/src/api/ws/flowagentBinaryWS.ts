...
import { MovingMessageBin, Signal } from "./FLOWAGENT_PROTO"
...
export class FLOWAGENTBinaryWebsocket {
  url: string
  ...
  status: FLOWAGENT_WEBSOCKET_STATUS = FLOWAGENT_WEBSOCKET_STATUS.INIT
  context: FLOWAGENT_WEBSOCKET_CONTEXT = FLOWAGENT_WEBSOCKET_CONTEXT.DASHBOARD

  constructor(url: string, context: FLOWAGENT_WEBSOCKET_CONTEXT) {
    this.url = url
    ...
    try {
      store.dispatch(
        configActions.updateWSStatusReducer({
          ...
          wsStatus: FLOWAGENT_WEBSOCKET_STATUS.CONNECTING,
        }),
      )
    ...
    if (this.ws) {
      ...
      this.ws.onopen = () => {
        ...
        store.dispatch(
          configActions.updateWSStatusReducer({
            context: this.context,
            wsStatus: FLOWAGENT_WEBSOCKET_STATUS.CONNECTED,
          }),
        )
        this.isOnline = true
        this.repeat = 0
