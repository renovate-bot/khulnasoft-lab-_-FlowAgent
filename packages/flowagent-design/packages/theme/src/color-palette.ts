...
import { globalColor, hasGlobalColor, flowagentPrefix } from "./global-color"
...
export const getColor = (color: string, step: string) => {
  ...
  if (!hasGlobalColor(`--${flowagentPrefix}-${color}-${step}`)) {
    ...
    colorStyle = globalColor(`--${flowagentPrefix}-${color}-${step}`)
  }
  return colorStyle
}
