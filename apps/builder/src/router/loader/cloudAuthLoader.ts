...
import { canAccessManage } from "@flowagent-public/user-role-utils"
import {
...
export const getUserInfoLoader: LoaderFunction = async () => {
  ...
  if (authToken) {
    try {
      ...
      FLOWAGENTMixpanel.getMixpanelInstance()?.identify(response.data.userID)
      const reportedUserInfo: Record<string, any> = {}
      Object.entries(response.data).forEach(([key, value]) => {
        reportedUserInfo[`flowagent_${key}`] = value
      })
      FLOWAGENTMixpanel.getMixpanelInstance()?.people.set(reportedUserInfo)
      store.dispatch(currentUserActions.updateCurrentUserReducer(response.data))
      return null
    } catch (e) {
      FLOWAGENTMixpanel.getMixpanelInstance()?.reset()
      return redirect("/403")
    }
  }
  return redirect("/403")
...
export const getTeamsInfoLoader: LoaderFunction = async (args) => {
  ...
  if (currentTeamInfo) {
    ...
    FLOWAGENTMixpanel.setGroup(teamIdentifier)
    if (
      isCloudVersion &&
      !canAccessManage(
      ...
      return redirect(
        `${getFLOWAGENTCloudURL()}/workspace/${currentTeamInfo.identifier}`,
      )
    }
    return null
  }
