import { ResourceCard, TeamContentEmpty } from "@flowagent-public/dashboard"
import { USER_ROLE } from "@flowagent-public/public-types"
import { ResourceTypeSelector } from "@flowagent-public/resource-generator"
import {
  getCurrentId,
  getCurrentTeamInfo,
  getPlanUtils,
...
export const PCResourcesWorkspace: FC = () => {
  ...
  const onSelectResourceType = (resourceType: string) => {
    window.open(
      `${getFLOWAGENTBuilderURL()}/${teamIdentifier}/resource/new/${resourceType}?token=${getAuthToken()}`,
    )
  }
          ...
          <div css={cardContainerStyle} onScroll={onContainerScroll}>
            {resourceListResult?.map((resourceInfo) => (
              <ResourceCard
                ...
                onEditResource={() => {
                  window.open(
                    `${getFLOWAGENTBuilderURL()}/${teamIdentifier}/resource/edit/${
