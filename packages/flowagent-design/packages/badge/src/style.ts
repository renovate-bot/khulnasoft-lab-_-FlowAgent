...
import { isObject } from "@flowagent-design/system"
import { BadgeColorScheme, BadgeStatus } from "./interface"
import { ReactNode } from "react"

const statusColor = {
  default: `--${flowagentPrefix}-grayBlue-06`,
  processing: `--${flowagentPrefix}-blue-03`,
  success: `--${flowagentPrefix}-green-03`,
  warning: `--${flowagentPrefix}-yellow-03`,
  error: `--${flowagentPrefix}-red-03`,
}
...
export function applyBadgeDot(
  ...
  return css`
    ...
    height: 6px;
    border-radius: 50%;
    border: solid 1px ${globalColor(`--${flowagentPrefix}-white-01`)};
...
export function applyBadgeNumberOrText(
  ...
  return css`
    ...
    border: solid 2px ${globalColor(`--${flowagentPrefix}-white-01`)};
    background-color: ${color};
...
export function getDotColor(
  ...
  colorStyle = colorStyle
    ? colorStyle
    : isObject(count)
    ? globalColor(`--${flowagentPrefix}-white-01`)
    : globalColor(`--${flowagentPrefix}-red-03`)
  return colorStyle
}
