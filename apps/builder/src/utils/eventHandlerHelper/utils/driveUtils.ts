...
import { ERROR_FLAG } from "@flowagent-public/flowagent-net/errorFlag"
import {
  CollarModalType,
...
interface IDownloadFromFLOWAGENTDriveParams {
  downloadInfo: IDriveDownloadInfo[]
  asZip?: boolean
}

export const downloadFromFLOWAGENTDrive = async (
  params: IDownloadFromFLOWAGENTDriveParams,
) => {
  ...
  const zipName = `flowagent_drive_download_${new Date().getTime()}.zip`
...
export interface ISaveToFLOWAGENTDriveParams {
  fileName: string
  fileData: string
  fileType: FILE_TYPE
  folder?: string
