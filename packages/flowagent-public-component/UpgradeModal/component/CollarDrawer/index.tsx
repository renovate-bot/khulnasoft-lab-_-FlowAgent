...
import { isMobileByWindowSize } from "@flowagent-public/utils"
...
import { COFLOWAGENTR_TYPE, PURCHASE_TYPE } from "../../interface"
import { cancelSubscribe, modifySubscribe, subscribe } from "../../service"
import {
  COFLOWAGENTR_UNIT_BY_CYCLE,
  COFLOWAGENTR_UNIT_PRICE,
...
import {
  COFLOWAGENTR_BUTTON_TEXT,
  COFLOWAGENTR_MORE_TEXT,
  LEARN_MORE_LINK,
} from "./constants"
...
export const CollarDrawer: FC<CollarDrawerProps> = (props) => {
  ...
  const isCancelSubscribe =
    currentTeamInfo?.colla?.plan === SUBSCRIBE_PLAN.COFLOWAGENT_SUBSCRIBE_CANCELED
  ...
  const currentCollarType = useRef<COFLOWAGENTR_TYPE>(
  ...
  const unitPrice = COFLOWAGENTR_UNIT_PRICE[cycle ?? SUBSCRIPTION_CYCLE.MONTHLY]
  const unitCollaByCycle =
    COFLOWAGENTR_UNIT_BY_CYCLE[cycle ?? SUBSCRIPTION_CYCLE.MONTHLY]
  ...
  const handleNumChange = (value?: number) => {
    ...
    if (currentTeamInfo?.colla?.cycle !== cycle && isSubScribe) {
      setBtnText(COFLOWAGENTR_BUTTON_TEXT.MONTH_YEAR_UPDATE)
      setDescription(COFLOWAGENTR_MORE_TEXT.MONTH_YEAR_UPDATE)
    } else {
  ...
  currentCollarType.current !== COFLOWAGENTR_TYPE.CANCEL_SUBSCRIPTION
  ...
  const handleSubscribe = async () => {
    ...
    const successRedirect = getSuccessRedirectWithParams({
      ...
      purchaseType: PURCHASE_TYPE.COFLOWAGENT,
      userID,
      ...
      switch (currentCollarType.current) {
        case COFLOWAGENTR_TYPE.CANCEL_SUBSCRIPTION:
          await cancelSubscribe(
            currentTeamInfo.id,
            currentTeamInfo.colla?.plan || SUBSCRIBE_PLAN.COFLOWAGENT_SUBSCRIBE_PAID,
          )
          ...
          break
        case COFLOWAGENTR_TYPE.MODIFY_SUBSCRIPTION:
          await modifySubscribe(currentTeamInfo.id, {
            plan: SUBSCRIBE_PLAN.COFLOWAGENT_SUBSCRIBE_PAID,
        ...
        case COFLOWAGENTR_TYPE.REMOVE_COFLOWAGENTR:
          await modifySubscribe(currentTeamInfo.id, {
            plan:
              ...
              SUBSCRIBE_PLAN.COFLOWAGENT_SUBSCRIBE_PAID,
            quantity: currentQuantity,
            cycle,
          })
          onSuccessCallback?.(currentTeamInfo.id, currentCollarType.current)
              ...
              <div css={managePriceStyle}>
                <span>
                  {t("billing.payment_sidebar.price.colla_monthly", {
                    price: `$${COFLOWAGENTR_UNIT_PRICE[SUBSCRIPTION_CYCLE.MONTHLY]}`,
                  })}
                </span>
                <span>
                  {t("billing.payment_sidebar.price.colla_yearly", {
                    price: `$${COFLOWAGENTR_UNIT_PRICE[SUBSCRIPTION_CYCLE.YEARLY]}`,
                  })}
                </span>
              </div>
            </div>
