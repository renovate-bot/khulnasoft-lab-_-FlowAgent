...
test.describe("Appwrite create And delete", () => {
  ...
  test("create Resource", async ({ page }) => {
    ...
    await page
      ...
      .fill(process.env.FLOWAGENT_APPWRITE_HOST)
    await page.getByPlaceholder("Database ID").click()
    await page
      .getByPlaceholder("Database ID")
      .fill(process.env.FLOWAGENT_APPWRITE_DATABASEID)
    await page.getByPlaceholder("Project ID").click()
    await page
      .getByPlaceholder("Project ID")
      .fill(process.env.FLOWAGENT_APPWRITE_PROJECTID)
    await page.getByPlaceholder("••••••••••••••••••••").click()
    await page
      .getByPlaceholder("••••••••••••••••••••")
      .fill(process.env.FLOWAGENT_APPWRITE_APIKEY)
