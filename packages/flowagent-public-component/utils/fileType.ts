...
export enum IFLOWAGENT_DRIVE_OBJECT_TYPE {
  IMAGE = "image",
  VIDEO = "video",
  AUDIO = "audio",
  PDF = "pdf",
...
export const getFileTypeByContentType = (
  ...
  if (type === GCS_OBJECT_TYPE.FOLDER) {
    return IFLOWAGENT_DRIVE_OBJECT_TYPE.FOLDER
  } else if (type === GCS_OBJECT_TYPE.ANONYMOUS_FOLDER) {
    return IFLOWAGENT_DRIVE_OBJECT_TYPE.ANONYMOUS_FOLDER
  }
  if (!contentType) {
    return IFLOWAGENT_DRIVE_OBJECT_TYPE.DEFAULT
  }
  if (matchRules(IMAGE_FILE_TYPE_RULES, contentType)) {
    return IFLOWAGENT_DRIVE_OBJECT_TYPE.IMAGE
  }
  if (matchRules(VIDEO_FILE_TYPE_RULES, contentType)) {
    return IFLOWAGENT_DRIVE_OBJECT_TYPE.VIDEO
  }
  if (matchRules(AUDIO_FILE_TYPE_RULES, contentType)) {
    return IFLOWAGENT_DRIVE_OBJECT_TYPE.AUDIO
  }
  if (matchRules(PDF_FILE_TYPE_RULES, contentType)) {
    return IFLOWAGENT_DRIVE_OBJECT_TYPE.PDF
  }
  if (matchRules(WORD_FILE_TYPE_RULES, contentType)) {
    return IFLOWAGENT_DRIVE_OBJECT_TYPE.WORD
  }
  if (matchRules(EXCEL_FILE_TYPE_RULES, contentType)) {
    return IFLOWAGENT_DRIVE_OBJECT_TYPE.EXCEL
  }
  if (matchRules(PPT_FILE_TYPE_RULES, contentType)) {
    return IFLOWAGENT_DRIVE_OBJECT_TYPE.PPT
  }
  if (matchRules(ZIP_FILE_TYPE_RULES, contentType)) {
    return IFLOWAGENT_DRIVE_OBJECT_TYPE.ZIP
  }
  return IFLOWAGENT_DRIVE_OBJECT_TYPE.DEFAULT
}

export const CAN_PREVIEW_FILE_TYPE = [
  IFLOWAGENT_DRIVE_OBJECT_TYPE.IMAGE,
  IFLOWAGENT_DRIVE_OBJECT_TYPE.VIDEO,
  IFLOWAGENT_DRIVE_OBJECT_TYPE.AUDIO,
]
