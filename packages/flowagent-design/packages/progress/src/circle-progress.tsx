...
import { SuccessIcon, WarningIcon } from "@flowagent-design/icon"
import {
  applyContainer,
...
export const CircleProgress = forwardRef<HTMLDivElement, ProgressProps>(
  (props, ref) => {
    ...
    switch (status) {
      case "success":
        statusComponent = (
          <SuccessIcon color={globalColor(`--${flowagentPrefix}-green-03`)} />
        )
        break
      case "error":
        statusComponent = (
          <WarningIcon color={globalColor(`--${flowagentPrefix}-red-03`)} />
        )
        break
    }
