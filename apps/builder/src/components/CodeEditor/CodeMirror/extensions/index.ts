...
import { buildIllaContextCompletionSource } from "@/components/CodeEditor/CodeMirror/extensions/completionSources/flowagentContext"
...
import { FLOWAGENTEditorRuntimePropsCollectorInstance } from "@/utils/executionTreeHelper/runtimePropsCollector"
...
const buildCompletionSources = (
  ...
  const flowagentSources = buildIllaContextCompletionSource(
    canShowCompleteInfo,
    codeType,
    executedResult,
  )
  const completionSources = [ternSource, flowagentSources]
...
export const useBasicSetup = (options: ICodeMirrorOptions) => {
  ...
  const executedResult =
    scopeOfAutoComplete === "global"
      ? FLOWAGENTEditorRuntimePropsCollectorInstance.getGlobalCalcContextWithLimit()
      : FLOWAGENTEditorRuntimePropsCollectorInstance.getCurrentPageCalcContext()
  ...
  const tooltipExtension = useMemo(() => {
    return tooltips({
      position: "absolute",
      parent:
        document.querySelector<HTMLElement>(".flowagentCodeMirrorWrapper") ||
        document.body,
    })
  }, [])
