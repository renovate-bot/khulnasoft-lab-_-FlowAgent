...
import { IAdvancedConfig } from "@flowagent-public/public-types"
import { FC, useCallback, useContext } from "react"
import { useTranslation } from "react-i18next"
import { useDispatch, useSelector } from "react-redux"
import { Switch } from "@flowagent-design/react"
...
export const AdvancedOptionSetting: FC = () => {
  ...
  const handleCodeMirrorFocus = useCallback(
    (attrName: string) => {
      return () => {
        track(FLOWAGENT_MIXPANEL_EVENT_TYPE.FOCUS, {
  ...
  const handleCodeMirrorBlur = useCallback(
    (attrName: string) => {
      return (value: string) => {
        track(FLOWAGENT_MIXPANEL_EVENT_TYPE.BLUR, {
          element: "advanced_code_mirror",
          parameter2: attrName,
          parameter3: value,
        })
